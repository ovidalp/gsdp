...
# Empty Image for type validation
emptyPIL = Image.new('L', (28, 28))
...
class Extractor(object):
    ...
    def _input_preprocessing(self, img, from_path=False):
        ...
        if from_path:
            ...
            if img is not None:
                # if PIL image
                if type(img) == type(emptyPIL):

                    img = img.resize(self.config.input_shape_2D)
                    numpy_image = img_to_array(img)

                else:  # opencv is in numpy format
                    img = cv2.resize(img, self.config.input_shape_2D)
                    tensor_shape = self.config.input_shape
                    numpy_image = img.reshape(tensor_shape[0], tensor_shape[1], tensor_shape[2])
